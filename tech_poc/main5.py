...
path = 'output/version_5'

def get_tech_solution(client : OpenAI, query: str, rewrite_assistant_id: str):
    prompt = """You are an expert in technology, specializing in prompt fine-tuning and technical solution design. Your task is to transform the user's request into a comprehensive technical solution. Begin with a concise overview of the solution enclosed in <tech> tags. Then, break down the solution into clear, manageable steps, each enclosed in <step> tags. Keep each step brief and focused, avoiding lengthy explanations or code snippets. Ensure the steps are practical and easy to understand, providing a high-level roadmap for implementation."""
